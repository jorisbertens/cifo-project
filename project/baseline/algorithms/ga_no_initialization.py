...
class GeneticAlgorithmNoInitialization(RandomSearch):
    ...
    def initialize(self):
        self.population = self._generate_0_valid_solutions()
    ...
    def _generate_0_valid_solution(self):
        solution = Solution(np.zeros(self.problem_instance.search_space[2]))
        self.problem_instance.evaluate(solution)
        return solution

    def _generate_0_valid_solutions(self):
        solutions = np.array([self._generate_0_valid_solution()
                              for i in range(self.population_size)])
        return solutions