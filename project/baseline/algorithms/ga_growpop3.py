...
class GeneticAlgorithmGrowPop3(RandomSearch):
    ...
    def search(self, n_iterations, report=False, log=False):
        ...
        while len(self.population) < self.max_pop_size:
            ...
            while len(offsprings) < len(self.population):
                off1, off2, off3 = p1, p2, p3 = [
                    self.selection(self.population, self.problem_instance.minimization, self._random_state) for _ in range(3)]
        ...
        for iteration in range(n_iterations):
            ...
            if log:
                log_event = [iteration, elite.fitness, elite.validation_fitness if hasattr(off3, 'validation_fitness') else None,
