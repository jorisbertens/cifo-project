...
class GeneticAlgorithmDeterministicCrowding(RandomSearch):
    '''
        https://www.researchgate.net/figure/Pseudocode-of-the-genetic-algorithm-based-on-deterministic-crowding-used-for-PAF_fig5_7290874?fbclid=IwAR1JALzlx9wJhtjicF-6vpCbGM3V_F4BpD7YbhW07QmB-7XLFe5FcA1xh3U
    '''
        ...
        for iteration in range(n_iterations):
            ...
            while len(offsprings) < pop_size:
                ...
                if self._random_state.uniform() < self.p_c:
                    off1, off2 = self._crossover(off1, off2)
                ...
                if (self._euclidian_distance(p1.representation,off1.representation) + self._euclidian_distance(p2.representation,off2.representation)) <= (
                    ...
                    offsprings.extend([off1]) if off1.fitness > p1.fitness else offsprings.extend([p1])
                    offsprings.extend([off2]) if off2.fitness > p2.fitness else offsprings.extend([p2])
                else:
