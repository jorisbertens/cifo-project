...
class GeneticAlgorithmMatingPool(RandomSearch):
    def __init__(self, problem_instance, random_state, population_size,
                 selection, crossover, p_c, mutation, p_m, mating_pool_size):
    ...
    def select_mating_pool(self, population, minimization, mating_pool_size):
        n_individ = int(len(population) * mating_pool_size)
        return sorted(population, key=lambda x: x.fitness, reverse=not minimization)[n_individ:]
