...
def algo_run(seed, n_gen, control, update_rate, radius, p, std, mean):
    ...
    #++++++++++++++++++++++++++
    mut_algo = mut.parametrized_gaussian_member_mutation(p, mean, std)
    ...
    result_string = ",".join(
        [str(alg.best_solution.fitness), str(accuracy),
         ...
         str(control), str(update_rate), str(radius), str(p), str(mean), str(std), str(time_elapsed)
         ])
...
if __name__ ==  '__main__':
    possible_values = list(itertools.product(*[seeds_per_run,n_genes,controls, update_rates, radiuses, ps, means, stds ]))
